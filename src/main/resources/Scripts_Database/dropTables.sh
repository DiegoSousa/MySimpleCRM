...
echo "=============================================="
echo "Drop Tables from MessengerConcurrent..."
echo
...
echo
psql -h localhost -U postgres -d messengerConcurrent -t -c "SELECT 'DROP TABLE ' || n.nspname || '.' || c.relname || ' CASCADE;' FROM pg_catalog.pg_class AS c LEFT JOIN pg_catalog.pg_namespace AS n ON n.oid = c.relnamespace WHERE relkind = 'r' AND n.nspname NOT IN ('pg_catalog', 'pg_toast') AND pg_catalog.pg_table_is_visible(c.oid)" > /tmp/droptables
echo
echo "Dropping all tables. Please provide a password if required: "
echo
psql -h localhost -d messengerConcurrent -U postgres -f /tmp/droptables
echo
